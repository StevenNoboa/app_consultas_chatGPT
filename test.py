...
def test_analizar_documento_route(client):
    ...
    with open(ruta,'rb') as archivo:
        response = client.post('/analizar_documento', data={'archivo': archivo, 'nombre': 'John', 'pregunta': '¿Qué edad tiene nuestro amigo Viggo?'})
    assert response.status_code == 200
...
def test_realizar_consulta_route(client):
    response = client.get('/realizar_consulta?nombre=Anuel')
    assert response.status_code == 200